...
public class ProjectileExplosion extends Explosion {
    ...
    public ProjectileExplosion(Level level, Entity owner, Entity exploder, @Nullable DamageSource source, @Nullable ExplosionDamageCalculator damageCalculator, double x, double y, double z, float power, float radius, boolean knockback, Explosion.BlockInteraction mode) {
        super(
                level,
                exploder,
                source != null ? source : level.damageSources().explosion(exploder, owner),
                damageCalculator != null ? damageCalculator : DEFAULT_CONTEXT,
                x, y, z,
                radius,
                AmmoConfig.EXPLOSIVE_AMMO_FIRE.get(),
                mode,
    ...
    public void explode() {
        ...
        for (Entity entity : entities) {
            ...
            entity.hurt(entity.damageSources().explosion(this), (float) damage * this.power);

            if (entity instanceof LivingEntity) {
                AttributeMap aMap = ((LivingEntity) entity).getAttributes();
                damage *= aMap.hasAttribute(Attributes.EXPLOSION_KNOCKBACK_RESISTANCE) ? aMap.getValue(Attributes.EXPLOSION_KNOCKBACK_RESISTANCE) : 1.0D;
            }
